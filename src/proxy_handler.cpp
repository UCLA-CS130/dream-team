...
RequestHandler::Status ProxyHandler::HandleRequestHelper(const Request& request,
  ...
  try {
    ...
    SendRequest(request, response, count, &socket);

    RequestHandler::Status head_stat = 
      HandleHeaders(request, response, count, &socket, &body_);

    //If we get a bad config value, or if the response body is set by
    // a recursive call
    if (head_stat != RequestHandler::OK || !response->GetBody().empty()) {
      return head_stat;
    }

    HandleBody(request, response, &body_);
...
void ProxyHandler::SendRequest(const Request& request, 
                               Response* response, 
                               int count, 
...
RequestHandler::Status ProxyHandler::HandleHeaders(const Request& request, 
                                                   Response* response, 
                                                   int count, 
  ...
  size_t index;
  size_t cr;
  bool redirected = (status_code == 302 || status_code == 301);
  std::string new_host = "";
  ...
  while (std::getline(response_stream, header) && header != "\r") {
    ...
    if (index != std::string::npos) {
      if (header.substr(0, index).compare("Location") || !redirected) {
...
void ProxyHandler::HandleBody(const Request& request, 
                              Response* response, 
                              std::string* body) {
  if (uri_prefix_.size() > 1) { 
